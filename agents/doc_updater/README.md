...
```
5. FINAL:
    - Si aceptamos → `APPLIER` modifica los services correspondientes → si ya existe una carpeta de releases asociada al día de aceptación pasa la US de `/done` a dicha carpeta y si no la crea automáticamente.
    - Si rechazamos → FIN y guardamos el motivo en el registro de rechazos rejections_log.json
...
# Configurar API key en el bashrc
```
