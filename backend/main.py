...
chat_state = {
    ...
    "doc_updater_waiting_decision": False,  # Esperando si/no del usuario para doc_updater
}
...
def upload_md(file: UploadFile = File(...)):
    ...
    # --- Ejecutar agente de creación de US ---
    try:
        cwd = os.getcwd()
...
def chat(msg: Message):
    ...
    if text == "exit" or text == "quit":
        ...
        chat_state["current_meeting_date"] = None
        chat_state["doc_updater_waiting_decision"] = False
        return {
    ...
    # --- Esperando decisión para doc_updater ---
    if chat_state["doc_updater_waiting_decision"]:
        chat_state["doc_updater_waiting_decision"] = False
        decision = "accept" if text in ["si", "yes", "ok", "s", "y"] else "reject"
        
        # Ejecutar el agent con la decisión (el propio agente aplicará o rechazará cambios)
        try:
            cwd = os.getcwd()
            os.chdir("agents/doc_updater")
            result = subprocess.run(
                ["python3", "doc_update_agent.py", f"--decision={decision}"],
                capture_output=True,
                text=True,
                timeout=300
            )
            ...
            if result.returncode == 0:
                if decision == "accept":
                    return {"response": "✅ Cambios aplicados correctamente"}
                else:
                    return {"response": "❌ Cambios rechazados"}
            else:
                return {"response": f"❌ Error aplicando/rechazando cambios: {result.stderr or result.stdout}"}
        except Exception as e:
    ...
    if option == "opciones":
        ...
        # Ejecutar agent en modo preview para generar propuestas
        try:
            cwd = os.getcwd()
            os.chdir("agents/doc_updater")
            result = subprocess.run(
                ["python3", "doc_update_agent.py", "--preview"],
                capture_output=True,
                text=True,
                timeout=300
            )
            ...
            if result.returncode != 0:
                ...
                if "No hay" in error_output or "no hay" in error_output.lower():
                    return {"response": "⚠️ No hay User Stories completadas en /backlog/done/"}
                return {"response": f"❌ Error: {error_output[:200]}"}
            
            # El agent ha impreso las propuestas en stdout
            response_text = result.stdout.strip()
            
            # Marcar que estamos esperando decisión
            chat_state["doc_updater_waiting_decision"] = True
            
            return {"response": f"{response_text}\n\n¿Deseas aplicar estos cambios? (si/no)"}
        except Exception as e:
            return {"response": f"❌ Error: {str(e)}"}
import re, shutil, os, datetime, markdown, json, subprocess
from pathlib import Path
from fastapi import FastAPI, UploadFile, File, HTTPException
from pydantic import BaseModel
from fastapi.middleware.cors import CORSMiddleware
...
def chat(msg: Message):
    ...
    if option == "opciones":
        ...
        # Ejecutar agent en modo chat para generar propuestas
        try:
            cwd = os.getcwd()
            os.chdir("agents/doc_updater")
            result = subprocess.run(
